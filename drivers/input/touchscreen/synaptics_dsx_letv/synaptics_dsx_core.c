...
#ifdef X1_WAKEUP_GESTURE_ENABLE
/*
#define WAKEUP_GESTURE true
*/

#define GESTURE_OFFSET 12
...
#define F01_PRODUCT_ID_OFFSET 11
...
static ssize_t synaptics_rmi4_wake_gesture_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_wake_gesture_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#ifdef X1_WAKEUP_GESTURE_ENABLE
static ssize_t synaptics_rmi4_double_click_wake_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_double_click_wake_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_down_to_up_wake_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_down_to_up_wake_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_up_to_down_wake_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_up_to_down_wake_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_right_to_left_wake_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t
synaptics_rmi4_right_to_left_wake_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_left_to_right_wake_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t
synaptics_rmi4_left_to_right_wake_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_detected_gesture_show(struct device *dev,
		struct device_attribute *attr, char *buf);
#endif
...
static struct device_attribute attrs[] = {
	...
	__ATTR(wake_gesture_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_wake_gesture_enable_show,
			synaptics_rmi4_wake_gesture_enable_store),
#ifdef X1_WAKEUP_GESTURE_ENABLE
	__ATTR(double_click_wake_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_double_click_wake_enable_show,
			synaptics_rmi4_double_click_wake_enable_store),
	__ATTR(down_to_up_wake_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_down_to_up_wake_enable_show,
			synaptics_rmi4_down_to_up_wake_enable_store),
	__ATTR(up_to_down_wake_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_up_to_down_wake_enable_show,
			synaptics_rmi4_up_to_down_wake_enable_store),
	__ATTR(right_to_left_wake_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_right_to_left_wake_enable_show,
			synaptics_rmi4_right_to_left_wake_enable_store),
	__ATTR(left_to_right_wake_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_left_to_right_wake_enable_show,
			synaptics_rmi4_left_to_right_wake_enable_store),
	__ATTR(detected_gesture, (S_IRUGO),
			synaptics_rmi4_detected_gesture_show,
			NULL),
#endif
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!queue_work(reset_workqueue, &reset_work)) {
		dev_err(rmi4_data->pdev->dev.parent,
			"reset_workqueue already queue\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
			rmi4_data->double_click_wake_enable);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->double_click_wake_enable = input;

	rmi4_data->enable_wakeup_gesture =
					rmi4_data->double_click_wake_enable ||
					rmi4_data->down_to_up_wake_enable ||
					rmi4_data->up_to_down_wake_enable ||
					rmi4_data->right_to_left_wake_enable ||
					rmi4_data->left_to_right_wake_enable;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->down_to_up_wake_enable = input;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->up_to_down_wake_enable = input;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->right_to_left_wake_enable = input;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->left_to_right_wake_enable = input;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			...
			input_sync(rmi4_data->input_dev);
			/*
			 * Reporting here is going to wakeup system, then
			 * synaptics_rmi4_resume() would be called,
			 * rmi4_data->suspend would be clear there.
			 */
...
flash_prog_mode:
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f01_query_base_addr + F01_PRODUCT_ID_OFFSET,
			rmi4_data->ModuleVendor,
			sizeof(rmi4_data->ModuleVendor));
	if (retval < 0)
		return retval;

	rmi4_data->ModuleVendor[6] = '\0';

	dev_err(rmi4_data->pdev->dev.parent, "Product id: %s\n",
			rmi4_data->ModuleVendor);
	...
	/*
	* Remove this useless code, it may cause kernel panic
	* */
	/*
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	mutex_lock(&(rmi4_data->rmi4_resume_mutex));
	retval = synaptics_rmi4_query_device(rmi4_data);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to query device\n",
				__func__);
		synaptics_rmi4_irq_enable(rmi4_data, true, false);
		mutex_unlock(&(rmi4_data->rmi4_resume_mutex));
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
	mutex_unlock(&(rmi4_data->rmi4_resume_mutex));
...
static void synaptics_rmi4_reset_fn_work(struct work_struct *work)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->double_click_wake_enable = 0;
	rmi4_data->down_to_up_wake_enable = 0;
	rmi4_data->up_to_down_wake_enable = 0;
	rmi4_data->right_to_left_wake_enable = 0;
	rmi4_data->left_to_right_wake_enable = 0;
	...
	mutex_init(&(rmi4_data->rmi4_resume_mutex));

	platform_set_drvdata(pdev, rmi4_data);

	i2c = to_i2c_client(rmi4_data->pdev->dev.parent);
	...
	if (vir_button_map->nbuttons) {
		rmi4_data->board_prop_dir = kobject_create_and_add(
				"synaptics_board_properties", NULL);
	...
	retval = sysfs_create_link(&i2c->dev.kobj,
			&rmi4_data->input_dev->dev.kobj, "touch");
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: Failed to create sysfs attributes link\n",
				__func__);
		goto err_sysfs;
	}
	...
	reset_workqueue = create_singlethread_workqueue("dsx_reset_workqueue");
	INIT_WORK(&reset_work, synaptics_rmi4_reset_fn_work);
...
#ifdef X1_WAKEUP_GESTURE_ENABLE
	unsigned char reporting_control_gesture[4];

	unsigned char gesture_type = 0x0;
#endif
	unsigned char doze_control;
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
...
#ifdef X1_WAKEUP_GESTURE_ENABLE
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + GESTURE_OFFSET,
			reporting_control_gesture,
			sizeof(reporting_control_gesture));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to change reporting mode\n",
				__func__);
...
#ifdef X1_WAKEUP_GESTURE_ENABLE
	if (rmi4_data->double_click_wake_enable)
		gesture_type |= 0x01;

	/*
	* Disable these code, because we only got double click
	* to resume system.
	* */
	/*
	if (rmi4_data->down_to_up_wake_enable)
		gesture_type |= rmi4_data->down_to_up_wake_enable;
	if (rmi4_data->up_to_down_wake_enable)
		gesture_type |= rmi4_data->up_to_down_wake_enable;
	if (rmi4_data->right_to_left_wake_enable)
		gesture_type |= rmi4_data->right_to_left_wake_enable;
	if (rmi4_data->left_to_right_wake_enable)
		gesture_type |= rmi4_data->left_to_right_wake_enable;
	*/

	rmi4_data->detected_gesture = DOUBLE;
	reporting_control_gesture[0] = gesture_type;
	retval = synaptics_rmi4_reg_write(rmi4_data,
			fhandler->full_addr.ctrl_base + GESTURE_OFFSET,
			reporting_control_gesture,
			sizeof(reporting_control_gesture));
...
static void fb_notify_resume_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data =
		 container_of(work,
			struct synaptics_rmi4_data, fb_notify_work);
				...
				unsigned long event, void *data)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent, "%s: Entry, event: %lu\n",
			__func__, event);
	if (evdata && evdata->data && rmi4_data && event == FB_EVENT_BLANK) {
		blank = evdata->data;
		dev_dbg(rmi4_data->pdev->dev.parent, "%s: blank %d\n",
				__func__, *blank);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend && rmi4_data->pwr_reg)
#else
	if (rmi4_data->pwr_reg)
#endif
		regulator_disable(rmi4_data->pwr_reg);
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->pwr_reg) {
#endif
		error = regulator_enable(rmi4_data->pwr_reg);
		msleep(bdata->power_delay_ms);
		rmi4_data->current_page = MASK_8BIT;
		if (rmi4_data->hw_if->ui_hw_init)
