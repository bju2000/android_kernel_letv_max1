...
ssize_t show_devtype(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct mhl_dev_context *dev_context = dev_get_drvdata(dev);

	if (dev_context->dev_type & DEV_TYPE_ARMET)
		return scnprintf(buf, PAGE_SIZE, "ARMET\n");
	else
		return scnprintf(buf, PAGE_SIZE, "TV\n");
}
			 ...
			 const char *buf, size_t count)
{
	...
	*/
	...
	if (idx == 0) {
		...
		MHL_TX_DBG_INFO("and length for subsequent scratch pad read\n");
	...
	u32 event_param, void *data)
{
	...
	case MHL_TX_EVENT_CONNECTION:
		...
		strlcpy(event_string, "MHL_EVENT=connected",
	...
	case MHL_TX_EVENT_DISCONNECTION:
		...
		*/
		if (dev_context->notify_disconnection)
			...
			MHL_TX_DBG_ERR(" OTG driver.\n");
#endif
		sysfs_notify(&dev_context->mhl_dev->kobj, NULL,
			__stringify(SYS_ATTR_NAME_CONN));

		strlcpy(event_string, "MHL_EVENT=disconnected",
			MAX_EVENT_STRING_LEN);
		kobject_uevent_env(&dev_context->mhl_dev->kobj, KOBJ_CHANGE,
			envp);
		break;
	...
	case MHL_TX_EVENT_RCP_RECEIVED:
		if (input_dev_rcp) {
			...
			MHL_TX_DBG_INFO("Get RCP rcvd event, keycode: 0x%02x\n",
							event_param);
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_RCP key code=0x%02x",
				event_param);
	...
	case MHL_TX_EVENT_RCPK_RECEIVED:
		if ((dev_context->mhl_flags & MHL_STATE_FLAG_RCP_SENT)
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_RCPK key code=0x%02x",
				event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		} else {
	...
	case MHL_TX_EVENT_RCPE_RECEIVED:
		if (input_dev_rcp) {
			...
			if (dev_context->mhl_flags & MHL_STATE_FLAG_RCP_SENT) {
				...
				snprintf(event_string, MAX_EVENT_STRING_LEN,
					"MHL_EVENT=rcvd_RCPE error code=0x%02x",
					event_param);
				kobject_uevent_env(&dev_context->mhl_dev->kobj,
					KOBJ_CHANGE, envp);
			} else {
				...
				MHL_TX_DBG_ERR("event, error code: 0x%02x\n",
					event_param);
			}
		}
		break;
	...
	case MHL_TX_EVENT_UCP_RECEIVED:
		if (input_dev_ucp) {
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_UCP key code=0x%02x",
				event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		}
	...
	case MHL_TX_EVENT_UCPK_RECEIVED:
		if ((dev_context->mhl_flags & MHL_STATE_FLAG_UCP_SENT) &&
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				 "MHL_EVENT=received_UCPK key code=0x%02x",
				 event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		} else {
	...
	case MHL_TX_EVENT_UCPE_RECEIVED:
		if (dev_context->mhl_flags & MHL_STATE_FLAG_UCP_SENT) {
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_UCPE error code=0x%02x",
				event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		} else {
	...
	case MHL_TX_EVENT_RBP_RECEIVED:
		if (input_dev_rbp) {
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_RBP button code=0x%02x",
				event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		}
	...
	case MHL_TX_EVENT_RBPK_RECEIVED:
		...
		if ((dev_context->mhl_flags & MHL_STATE_FLAG_RBP_SENT)
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				"MHL_EVENT=received_RBPK button code=0x%02x",
				event_param);
			kobject_uevent_env(&dev_context->mhl_dev->kobj,
				KOBJ_CHANGE, envp);
		} else {
	...
	case MHL_TX_EVENT_RBPE_RECEIVED:
		...
		if (input_dev_rbp) {
			...
			if (dev_context->mhl_flags & MHL_STATE_FLAG_RBP_SENT) {
				...
				snprintf(event_string, MAX_EVENT_STRING_LEN,
					"MHL_EVENT=rcvd_RBPE error code=0x%02x",
					event_param);
				kobject_uevent_env(&dev_context->mhl_dev->kobj,
					KOBJ_CHANGE, envp);
			} else {
	...
	case MHL_TX_EVENT_SPAD_RECEIVED:
		...
		idx = snprintf(event_string, MAX_EVENT_STRING_LEN,
			"MHL_EVENT=SPAD_CHG length=0x%02x data=", length);
	...
	case MHL_TX_EVENT_POW_BIT_CHG:
		...
		MHL_TX_DBG_INFO("event, POW bit is %s\n",
				event_param ? "ON" : "OFF");
		snprintf(event_string, MAX_EVENT_STRING_LEN,
			 "MHL_EVENT=MHL VBUS power %s",
			 event_param ? "ON" : "OFF");
		kobject_uevent_env(&dev_context->mhl_dev->kobj, KOBJ_CHANGE,
				   envp);
		break;
	...
	case MHL_TX_EVENT_RAP_RECEIVED:
		...
		MHL_TX_DBG_INFO("action code: 0x%02x\n", event_param);

		sysfs_notify(&dev_context->mhl_dev->kobj, NULL,
			     __stringify(SYS_ATTR_NAME_RAP_IN));

		snprintf(event_string, MAX_EVENT_STRING_LEN,
			 "MHL_EVENT=received_RAP action code=0x%02x",
			 event_param);
		kobject_uevent_env(&dev_context->mhl_dev->kobj, KOBJ_CHANGE,
				   envp);
		break;
	...
	case MHL_TX_EVENT_BIST_READY_RECEIVED:
		...
		snprintf(event_string, MAX_EVENT_STRING_LEN,
			 "MHL_EVENT=received_BIST_READY code=0x%02x",
			 event_param);
		kobject_uevent_env(&dev_context->mhl_dev->kobj, KOBJ_CHANGE,
				   envp);
		break;
	...
	case MHL_TX_EVENT_BIST_STATUS_RECEIVED:
		{
			...
			if (stat != NULL) {
				/*MHL_TX_DBG_INFO
					...
					stat->avlink_stat);
				snprintf(event_string, MAX_EVENT_STRING_LEN,
					"MHL_EVENT=received_BIST_STATUS "\
					"e_cbus_rx=0x%04x"
					"e_cbus_tx=0x%04x"
					" av_link=0x%04x",
					stat->e_cbus_local_stat,
...
struct device_attribute driver_attribs[] = {
	...
	__ATTR(devtype, 0666, show_devtype, NULL),
#ifdef DEBUG
			...
			struct si_adopter_id_data *p_burst)
{
	...
	case EDID_BLOCK: {
		...
		MHL_TX_DBG_INFO("EDID_BLOCK blk_num: %02X\n",
			     ...
			     void (*usb_notify_cb)(void *, int), void *ctx)
{
	...
	if (dw3_id_state != RID_MHL) {
		pr_info("OTG device event rid_state=%d.\n", dw3_id_state);
		return 1;
	}
	...
	rc = regulator_enable(mhl_reg);
	if (rc < 0)
		pr_err("%s: regulator_enable failed, rc=%d\n", __func__, rc);

	rc = regulator_set_optimum_mode(mhl_reg, 343000);
	if (rc < 0)
		pr_err("%s: set vcc mode failed, rc=%d\n", __func__, rc);

	dev_context->drv_info->mhl_device_initialize(
	...
	if (mhl_sii_wait_for_rgnd(dev_context)) {
		...
		pr_err("%s:shutdown PWR_1V0 of 8620 0000000\n", __func__);
	...
	if (rc) {
		...
		pr_err("%s:shutdown PWR_1V0 of 8620 111111111\n", __func__);
	}
				  ...
				  union power_supply_propval *val)
{
	...
	if (psy) {
		qualcomm_p_data = container_of
			(psy, struct qualcomm_platform_data, mhl_psy);
	...
	struct qualcomm_platform_data *pdata)
{
	...
	if (pdata->hdmi_pdev) {
		rc = msm_hdmi_register_mhl
			(pdata->hdmi_pdev, hdmi_mhl_ops, dev_context);
		if (rc)
			pr_err("%s: register with hdmi failed\n", __func__);
	}
	/*if (hdmi_mhl_ops->set_mhl_max_pclk)
		hdmi_mhl_ops->set_mhl_max_pclk(pdata->hdmi_pdev,75000);
		// Max pixel clock 75MHz*/
...
static int mhl_reboot(struct notifier_block *notify_block,
		unsigned long val, void *unused)
{

	struct mhl_dev_context *dev_context = container_of(
		notify_block, struct mhl_dev_context, mhl_notifier);
	struct drv_hw_context *hw_context =
		(struct drv_hw_context *)(&dev_context->drv_context);

	if (hw_context) {
		pr_err("wyh reboot mhl.\n");
		mhl_tx_modify_reg(hw_context, TX_PAGE_5 | 0xE6, 0xC0, 0x40);
	}
...
int mhl_tx_init(struct mhl_drv_info const *drv_info, struct device *parent_dev)
{
	...
	wake_lock_init(&dev_context->mhl_on_wake_lock, WAKE_LOCK_SUSPEND,
				"mhl_on_lock");
	...
	init_completion(&dev_context->rgnd_done);
	qualcomm_p_data = kzalloc
		(sizeof(struct qualcomm_platform_data), GFP_KERNEL);
	...
	ret = mhl_tx_get_platform_data
		(parent_dev, dev_context, qualcomm_p_data);
	if (ret) {
	...
	dev_context->mhl_notifier.notifier_call = mhl_reboot;
	ret = register_reboot_notifier(&dev_context->mhl_notifier);
	if (ret) {
		dev_err(parent_dev,
...
free_mem:
	wake_lock_destroy(&dev_context->mhl_on_wake_lock);
	kfree(dev_context);
