...
struct mxt_data {
	...
	int double_click_wake_enable;
	int down_to_up_wake_enable;
	int up_to_down_wake_enable;
	int right_to_left_wake_enable;
	int left_to_right_wake_enable;
...
#ifdef MAX1_WAKEUP_GESTURE_ENABLE
	} else if ((report_id == data->T115_reportid) && data->in_suspend) {
		mxt_proc_t115_messages(data, message);
	} else if (report_id == data->T93_reportid && data->in_suspend &&
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	struct t7_config deepsleep = { .active = 30, .idle = 30 };
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data->double_click_wake_enable)
		data->enable_wakeup_gesture++;
	else
		data->enable_wakeup_gesture--;

	if (data->enable_wakeup_gesture == 1)
		enable_irq_wake(data->client->irq);
	else if (data->enable_wakeup_gesture == 0)
...
static DEVICE_ATTR(sys_suspend, S_IWUSR,
	mxt_sys_suspend_show, mxt_sys_suspend_store);
	...
	struct mxt_config_info *info, bool force)
{
	...
	if (info->config_year > cfg_version->year || info->config_month > cfg_version->month
		...
		dev_info(dev, "%s, the same cfg version\n", __func__);
		return 0;
	}
