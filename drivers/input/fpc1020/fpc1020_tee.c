...
struct clk *blsp2_ahb_clk_p = NULL;
struct clk *blsp2_qup4_spi_clk_p = NULL;
bool clk_enable_status = false;
#endif
//add end by subin 
...
// disable regulator control, it is allways enabled
#if 0
...
//su add begain
#if 0
static int fpc1020_clk_set(struct device *dev, bool flag)
{
	int ret = 0;

	if ( (blsp2_ahb_clk_p != NULL) && (!IS_ERR(blsp2_ahb_clk_p)) ) {
		if (flag) {
			ret = clk_prepare_enable(blsp2_ahb_clk_p);
			if (ret) {
				dev_err(dev, "blsp2_ahb_clk enable fail\n");
				return ret;
			}
		} else {
			clk_disable_unprepare(blsp2_ahb_clk_p);
		}
	}

	if ( (blsp2_qup4_spi_clk_p != NULL) && (!IS_ERR(blsp2_qup4_spi_clk_p)) ) {
		if (flag) {
			ret = clk_prepare_enable(blsp2_qup4_spi_clk_p);
			if (ret) {
				dev_err(dev, "blsp2_qup4_spi_clk enable fail\n");
				return ret;
			}
		} else {
			clk_disable_unprepare(blsp2_qup4_spi_clk_p);
		}
	}
	return 0;
}
...
//add by subin 
#if 0
	blsp2_ahb_clk_p = clk_get(&spi->dev, "iface_clk");
	if (IS_ERR(blsp2_ahb_clk_p)) {
		dev_err(&spi->dev, "can't find blsp2_ahb_clk\n");
		goto err;
	}
	
	printk("FPC blsp2_ahb_clk_p get ok \n");

	blsp2_qup4_spi_clk_p = clk_get(&spi->dev, "core_clk");
	if (IS_ERR(blsp2_qup4_spi_clk_p)) {
		dev_err(&spi->dev, "can't find blsp2_qup4_spi_clk\n");
		goto err;
	}
	
	printk("FPC blsp2_qup4_spi_clk_p get ok \n");

	if (!clk_enable_status) {
		error = fpc1020_clk_set(&spi->dev, true);
		if (error) {
			goto err;
		} else {
			clk_enable_status = true;
			printk("FPC clk_enable_status set ok \n");
		}
	}
#endif
//add end by subin 
...
#if 0
err : 
	printk("FPC somethings is error\n");
	if ( (blsp2_ahb_clk_p != NULL) && (!IS_ERR(blsp2_ahb_clk_p)) ) {
		clk_put(blsp2_ahb_clk_p);
		blsp2_ahb_clk_p = NULL;
	}

	if ( (blsp2_qup4_spi_clk_p != NULL) && (!IS_ERR(blsp2_qup4_spi_clk_p)) ) {
		clk_put(blsp2_qup4_spi_clk_p);
		blsp2_qup4_spi_clk_p = NULL;
	}
	rc = -EINVAL;
	return rc;
#endif
