		...
		struct es9018_data *pdata)
{
	...
	if (pdata->ldo_0 < 0) {
		dev_err(dev, "Looking up %s property in node %s failed %d\n",
				"es9018,ldo_ena_92", dev->of_node->full_name,
				pdata->ldo_0);
		goto err;
	}
	dev_dbg(dev, "%s: ldo_ena_gpio_92 %d", __func__, pdata->ldo_0);

	pdata->ldo_1 = of_get_named_gpio(dev->of_node,
			"es9018,ldo_ena_96", 0);
	if (pdata->ldo_1 < 0) {
		dev_err(dev, "Looking up %s property in node %s failed %d\n",
				"es9018,ldo_ena_96", dev->of_node->full_name,
				pdata->ldo_1);
		goto err;
	}
	dev_dbg(dev, "%s: ldo_ena_gpio_96 %d", __func__, pdata->ldo_1);

	pdata->bypass_ctl = of_get_named_gpio(dev->of_node,
			"es9018,bypass_ctl_89", 0);
	if (pdata->ldo_1 < 0) {
		dev_err(dev, "Looking up %s property in node %s failed %d\n",
				"es9018,bypass_ctl_89", dev->of_node->full_name,
				pdata->bypass_ctl);
		goto err;
	}
	dev_dbg(dev, "%s: bypass_ctl_gpio_89 %d", __func__, pdata->bypass_ctl);
