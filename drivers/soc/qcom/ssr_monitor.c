...
static ssize_t ssr_monitor_read(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	ret = simple_read_from_buffer(buf, count, ppos, (void *)ssr_monitor_crash_reason, sizeof(ssr_monitor_crash_reason));
...
int ssr_monitor_notify(char* name)
{
	...
	if(strncmp(name, "ramdump_smem", sizeof("ramdump_smem"))){
		queue_work(ssr_md->ssr_monitor_wait_wq, &(ssr_md->ssr_monitor_work));
		pr_err("ssr_monitor_notify: %s dump.\n",name);
		ret = 0;
	}else{
		ret = 0;
	}
...
{
	pr_err("subsystem ssr is found in ssr_monitor\n");
...
static int ssrmonitor_setup_cdev(dev_t devno)
{
	...
	if (err) {
		pr_err("ssrmonitor cdev registration failed !\n");
		return -1;
	}
	...
	if (IS_ERR(ssr_md->ssrmonitor_class)) {
		pr_err("Error creating ssr_monitor class.\n");
		return -1;
	}
...
static int __init ssr_monitor_init(void)
{
	...
	if (!error) {
		...
		pr_err("ssr_monitor_init: major number not allocated.\n");
		goto FAILED;
	}

	ssr_md->cdev = cdev_alloc();
	if(!ssr_md->cdev){
		pr_err("ssr_monitor_init:cdev allocate failed.\n");
		goto FAILED;
	}
