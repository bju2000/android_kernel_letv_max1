...
int check_sii8620_mhl_connect_status(void)
{
	return MHL_CONNECTED;
}
EXPORT_SYMBOL(check_sii8620_mhl_connect_status);
...
static int mhl3_block_isr(struct drv_hw_context *hw_context, uint8_t int_status)
{
	...
	do {
		...
		if (req_size < 1) {
			MHL_TX_DBG_ERR("Header Error: Command size less than ");
			MHL_TX_DBG_ERR("STD Transaction Header Size: %d\n",
				req_size);
			goto done_emsc;
		}
		if (use_spi) {
		...
		if (req_size < remaining) {
			MHL_TX_DBG_ERR("Header Error: ");
			MHL_TX_DBG_ERR(
				"RFIFO_LEN: %d  REQ_LEN: %d REMAINING: %d\n",
		...
		if ((unload_cnt + remaining) == 0) {
			MHL_TX_DBG_ERR("Header Error:");
			MHL_TX_DBG_ERR(
				"Unload and Remaining counts are 0!\n");
			goto drain_req;
		}
		...
		if ((avail + unload_cnt) >
			...
			MHL_TX_DBG_ERR(
				"eMSC Header Error: unload count too large:");
			MHL_TX_DBG_ERR("%d + %d > %d\n",
				unload_cnt, avail,
				hw_context->block_protocol.peer_blk_rx_buf_max);
		...
		if (data_len < remaining) {
			...
			MHL_TX_DBG_WARN("byte count: %d < %d\n",
				data_len, remaining);
...
#define SWWA_BZ30759
		...
		if (0 == block_number) {
			...
			MHL_TX_DBG_INFO("Reg %02X:%02X to %02X\n",
				   ...
				   struct avi_info_frame_t *avif)
{
	...
	if (0 != checksum) {
		MHL_TX_DBG_ERR("AVI info frame checksum is:");
		MHL_TX_DBG_ERR("0x%02x should be 0\n", checksum);
		return 0;
			...
			union vsif_mhl3_or_hdmi_u *vsif)
{
	...
	if (0 != checksum) {
		MHL_TX_DBG_WARN("VSIF info frame checksum is: ");
		MHL_TX_DBG_WARN("0x%02x should be 0\n", checksum);
		/*
		...
		if (0 != checksum) {
			...
			MHL_TX_DBG_ERR("for checksum itself) is: ");
			MHL_TX_DBG_ERR("0x%02x should be 0\n", checksum);
			return 0;
...
static int set_hdmi_params(struct mhl_dev_context *dev_context)
{
	...
	if (hw_context->valid_vsif) {
		...
		if (0 == input_video_code.VIC) {
			...
			VIC -
			(size_t) &hw_context->current_avi_info_frame);
			MHL_TX_DBG_ERR("trying HTOTAL/VTOTAL\n");
	...
	if (qualify_pixel_clock_for_mhl(dev_context->edid_parser_context,
		...
		if (si_edid_sink_supports_YCbCr422(
			...
			if (qualify_pixel_clock_for_mhl(
				...
				MHL_TX_DBG_ERR("pixel clock too high %s\n",
			...
			MHL_TX_DBG_ERR("unsupported video mode.");
			MHL_TX_DBG_ERR("Sink doesn't support 4:2:2.\n");
			return false;
		}
	}
...
/* end BZ 32674 ) */
			...
			MHL_TX_DBG_ERR("available. Sink's link mode = 0x%02x\n",
				dev_context->dev_cap_cache.mdc.vid_link_mode);
			return false;
		}
	} else {
...
void conf_vsif_out(struct drv_hw_context *hw_context)
{
	enum mhl_vid_fmt_e vid_fmt = mhl_vid_fmt_no_additional;
	enum mhl_3d_fmt_type_e _3d_fmt_type = MHL_3D_FMT_TYPE_FS;
	enum mhl_sep_audio_e sep_aud = mhl_sep_audio_not_available;
	...
	MHL_TX_DBG_WARN("conf_vsif_out,VSIF from data islands\n")

	if (hvf3DFormatIndicationPresent ==
			hw_context->current_vsif.hdmi.payLoad.pb4.HDMI_Video_Format) {
			vid_fmt = mhl_vid_fmt_3d_fmt_present;
			MHL_TX_DBG_INFO("conf_vsif_out,VSIF indicates 3D\n");
			switch (hw_context->current_vsif.hdmi.payLoad.pb5.
				ThreeDStructure.threeDStructure) {
			case tdsTopAndBottom:
				_3d_fmt_type = MHL_3D_FMT_TYPE_TB;
				break;
			case tdsSideBySide:
				_3d_fmt_type = MHL_3D_FMT_TYPE_LR;
				break;
			default:
				break;
			}
	...
	struct avi_info_frame_t *avif)
{
	...
	if (NULL != vsif) {
		if (is_valid_vsif(dev_context, vsif)) {
			...
			if(dev_context->dev_type == DEV_TYPE_ARMET)
				conf_vsif_out(hw_context);
			else
				mode_change = true;
		}
	}
...
{
	uint8_t reg_val;
	MHL_TX_DBG_INFO("si_mhl_tx_drv_set_upstream_edid() called\n");
	...
	if (BIT_TMDS_CSTAT_P3_SCDT & reg_val) {
		...
		MHL_TX_DBG_INFO("Simulate int_5: 0x%02x\n",
...
static int hdcp2_isr(struct drv_hw_context *hw_context, uint8_t intr_status)
{
	...
	if (intr_status & BIT_HDCP2_INTR_AUTH_FAIL) {
		...
		MHL_TX_DBG_ERR(" - gp0 %02X, status %02X %02X\n",
...
static void switch_to_idle(struct drv_hw_context *hw_context,
			 bool do_interrupt_clear)
{
	MHL_TX_DBG_INFO("switch_to_idle\n\n");
...
static int int_4_isr(struct drv_hw_context *hw_context, uint8_t int_4_status)
{
	...
	if ((BIT_CBUS_MHL12_DISCON_INT & int_4_status) ||
		...
		if (mhl_clk_base)
			clk_disable_unprepare(mhl_clk_base);
		switch (hw_context->cbus_mode) {
		case CM_TRANSITIONAL_TO_eCBUS_S_BIST:
		case CM_TRANSITIONAL_TO_eCBUS_D_BIST:
		case CM_TRANSITIONAL_TO_eCBUS_S_CAL_BIST:
		...
		if (wake_lock_active(&dev_context->mhl_on_wake_lock))
			wake_unlock(&dev_context->mhl_on_wake_lock);
		/* For proper MHL discovery and tolerance of impedance
		 * measurement, set 5E3[7:4]=0x01 - this turns off the
		 * CBUS pull up for discovery states and 20K for IDLE
		 * state before impedance is measured. Fixes CBUS CTS
		...
		rc = regulator_disable(mhl_reg);
		if (rc < 0)
			pr_err("%s: regulator_enable failed, rc=%d\n",
				__func__, rc);
		si_mhl_tx_shutdown(dev_context);
		...
		if (qualcomm_p_data->hdmi_mhl_ops &&
			qualcomm_p_data->hdmi_mhl_ops->set_upstream_hpd)
			qualcomm_p_data->hdmi_mhl_ops->
				set_upstream_hpd(qualcomm_p_data->hdmi_pdev, 0);
		...
		if (VAL_RGND_1K == disc_stat2) {
			if (!wake_lock_active(&dev_context->mhl_on_wake_lock))
				wake_lock(&dev_context->mhl_on_wake_lock);
			mhl_tx_vbus_control(VBUS_ON);
			MHL_TX_DBG_WARN("Cable impedance = 1k (MHL Device)\n");
			/*added by zhang yue for qualcomm platform start*/
			MHL_TX_DBG_ERR("set_upstream_hpd start\n");

			if (qualcomm_p_data->hdmi_mhl_ops &&
				qualcomm_p_data->
					hdmi_mhl_ops->set_upstream_hpd){
				qualcomm_p_data->hdmi_mhl_ops->
					set_upstream_hpd(
					qualcomm_p_data->hdmi_pdev, 1);
				MHL_TX_DBG_ERR("set_upstream_hpd end\n");
			}
	...
	if (intr_stat) {
		...
		if (BIT_MDT_XMIT_SM_ABORT_PKT_RCVD & intr_stat) {
			MHL_TX_DBG_ERR
			    ...
			    ANSI_ESC_RED_TEXT);
			    MHL_TX_DBG_ERR("- status: 0x%02x%s\n",
			     mhl_tx_read_reg(hw_context,
			       REG_MDT_SM_STAT),
			     ANSI_ESC_RESET_TEXT);
		}
	...
	struct interrupt_info *intr_info)
{
	...
	MHL_TX_DBG_INFO("si_mhl_tx_drv_device_isr() called.");
	MHL_TX_DBG_INFO("mhl_tx_read_reg(hw_context, REG_COC_STAT_0)=0x%x\n\n",
		mhl_tx_read_reg(hw_context, REG_COC_STAT_0));
...
int si_mhl_tx_chip_initialize(struct drv_hw_context *hw_context)
{
	...
	pr_err("si_mhl_tx_chip_initialize........\n");

	gpio_set_value(GPIO_BB_FW_WAKE, 1);
...
{
	gpio_set_value(GPIO_BB_FW_WAKE, 0);
}
	...
	*dev_context, uint32_t pixel_clock_frequency, uint8_t bits_per_pixel)
{
	...
	case VAL_TX_ZONE_CTL3_TX_ZONE_6GBPS:
		...
		mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL1,
			0xBB);
		mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL6,
			0x3f);
		mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL7,
			0x03);
				  ...
				  struct bist_setup_info *test_info)
{
	...
	if (dev_context->bist_trigger_info & BIST_TRIGGER_TEST_E_CBUS_D) {
		/*MHL_TX_DBG_INFO("eCBUS State Machine:"
				       ...
				       struct bist_setup_info *test_info)
{
	...
	default:
		/*MHL_TX_DBG_ERR
			("Invalid value 0x%02x "
			"specified in IMPEDANCE_MODE field\n",
			test_info->impedance_mode);*/
		status = -EINVAL;
	}
	return status;
