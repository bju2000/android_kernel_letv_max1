		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(report_type, sizeof(int), "%d", F54_TX_TO_TX_SHORTS);
	test_sysfs_read_report_store(dev, attr, report_type,
					sizeof(report_type));

	for (ii = 0; ii < f54->report_size; ii++) {
		if (f54->report_data[ii]) {
			return snprintf(buf, PAGE_SIZE,
					"TP Short Circuit Detected,\nType-5, %03d: 0x%02x\n",
					ii, f54->report_data[ii]);
		}
	}
	snprintf(report_type, sizeof(F54_TRX_SHORTS), "%d",
				F54_TRX_SHORTS);
	test_sysfs_read_report_store(dev, attr, report_type,
					sizeof(report_type));
...
short open_circuit_threshold_lens[13][24] = {
	{3328, 3295, 3292, 3269, 3255, 3245, 3236, 3224, 3215, 3210, 3201,
		3210, 2880, 2868, 2857, 2840, 2826, 2819, 2807, 2798, 2791,
		2786, 2722, -2},
	{3568, 3537, 3514, 3497, 3476, 3462, 3438, 3420, 3401, 3387, 3370,
		3354, 3219, 3203, 3191, 3172, 3163, 3153, 3144, 3139, 3128,
		3125, 3064, 73},
	{3552, 3516, 3495, 3476, 3453, 3434, 3410, 3389, 3363, 3347, 3330,
		3311, 3205, 3194, 3170, 3151, 3139, 3130, 3123, 3111, 3104,
		3106, 3062, 2847},
	{3552, 3519, 3495, 3476, 3455, 3434, 3413, 3389, 3365, 3342, 3330,
		3311, 3231, 3215, 3205, 3191, 3179, 3170, 3163, 3156, 3149,
		3153, 3073, 164},
	{3535, 3504, 3479, 3462, 3434, 3408, 3389, 3368, 3342, 3321, 3302,
		3283, 3231, 3224, 3212, 3201, 3184, 3172, 3163, 3163, 3153,
		3156, 3085, 129},
	{3540, 3509, 3483, 3462, 3436, 3413, 3389, 3363, 3340, 3316, 3297,
		3278, 3248, 3241, 3226, 3212, 3203, 3194, 3186, 3196, 3179,
		3177, 3102, 160},
	{3528, 3490, 3464, 3443, 3410, 3394, 3361, 3335, 3307, 3288, 3271,
		3248, 3241, 3231, 3219, 3222, 3198, 3191, 3175, 3172, 3170,
		3170, 3111, 2845},
	{3530, 3497, 3471, 3448, 3422, 3398, 3370, 3347, 3321, 3297, 3281,
		3259, 3281, 3274, 3264, 3257, 3248, 3238, 3229, 3224, 3217,
		3222, 3139, 169},
	{3526, 3488, 3460, 3441, 3415, 3394, 3365, 3342, 3316, 3292, 3274,
		3252, 3302, 3292, 3292, 3271, 3262, 3255, 3245, 3241, 3238,
		3238, 3161, 131},
	{3516, 3483, 3457, 3438, 3406, 3380, 3356, 3330, 3304, 3285, 3269,
		3248, 3318, 3309, 3302, 3295, 3283, 3290, 3276, 3274, 3267,
		3269, 3191, 160},
	{3504, 3476, 3446, 3420, 3394, 3370, 3347, 3323, 3297, 3278, 3252,
		3234, 3347, 3323, 3311, 3307, 3295, 3295, 3290, 3283, 3278,
		3278, 3217, 2885},
	{3507, 3476, 3446, 3424, 3401, 3377, 3361, 3332, 3311, 3288, 3271,
		3255, 3387, 3375, 3365, 3358, 3351, 3351, 3344, 3342, 3335,
		3347, 3248, 148},
	{3172, 3130, 3097, 3080, 3057, 3033, 3010, 2986, 2960, 2941, 2927,
		2909, 3262, 3229, 3219, 3208, 3194, 3186, 3184, 3184, 3182,
		3189, 3132, 2116}
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(report_type, sizeof(F54_FULL_RAW_CAP_NO_RX_COUPLING), "%d",
				F54_FULL_RAW_CAP_NO_RX_COUPLING);
	test_sysfs_read_report_store(dev, attr, report_type,
					sizeof(report_type));

	report_data_16 = (short *)f54->report_data;
	tmp_id = f54->rmi4_data->rmi4_mod_info.product_id_string;
	if (!strcmp(tmp_id, "X1_L03")) {
		tmp = open_circuit_threshold_lens;
	} else if (!strcmp(tmp_id, "X1_O01")) {
		/*
		* FAE didn't gave thr raw_data of Ofilm,
		* I'll add them later.
		* */
		return snprintf(buf, PAGE_SIZE, "Pass\n");
	} else
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
			limit_b = tmp[ii][jj] * (100 + ADD_MINUS_PERSENT) / 100;
			limit_l = tmp[ii][jj] * (100 - ADD_MINUS_PERSENT) / 100;
			if (!(jj == 23 && (ii == 0 || ii == 1 || ii == 3
				|| ii == 4 || ii == 5 || ii == 7
				|| ii == 8 || ii == 9 || ii == 11
				|| ii == 12))) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(report_type, sizeof(F54_FULL_RAW_CAP_NO_RX_COUPLING), "%d",
				F54_FULL_RAW_CAP_NO_RX_COUPLING);
	test_sysfs_read_report_store(dev, attr,
					report_type, sizeof(report_type));
	report_data_16 = (short *)f54->report_data;
	tmp_id = f54->rmi4_data->rmi4_mod_info.product_id_string;

	if (strcmp(tmp_id, "X1_L03") && strcmp(tmp_id, "X1_O01"))
		return snprintf(buf, PAGE_SIZE,
			"Invalid product id:%s\n", tmp_id);
	...
	if (!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string,
		"X1_O01")){
		cnt = snprintf(buf, PAGE_SIZE - count,
		...
		buf += cnt;
		count += cnt;
	} else if (!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string,
		"X1_L03")){
		cnt = snprintf(buf, PAGE_SIZE - count,
				"max = %d, min = %d, average = %d\n",
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if ((f54->data_pos + count) > f54->report_size)
		...
		read_size = count;
		read_size = min(read_size, f54->report_size);
	}
