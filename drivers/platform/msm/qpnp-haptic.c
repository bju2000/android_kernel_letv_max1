...
static void qpnp_hap_set_level(struct qpnp_hap *hap, int level)
{
	u8 reg = 0;
	int temp;
	...
	qpnp_hap_read_reg(hap, &reg, QPNP_HAP_VMAX_REG(hap->base));
	reg &= QPNP_HAP_VMAX_MASK;
	reg |= (temp << QPNP_HAP_VMAX_SHIFT);
	qpnp_hap_write_reg(hap, &reg, QPNP_HAP_VMAX_REG(hap->base));
}
...
static void qpnp_hap_td_level(struct timed_output_dev *dev, int level)
{
	struct qpnp_hap *hap = container_of(dev, struct qpnp_hap, timed_dev);

	if (level > 0)
		qpnp_hap_set_level(hap, level);

}

static int qpnp_hap_td_get_level(struct timed_output_dev *dev)
{
	u8 reg = 0;
	int rc;
	struct qpnp_hap *hap = container_of(dev, struct qpnp_hap, timed_dev);
...
static int qpnp_haptic_probe(struct spmi_device *spmi)
{
	...
	hap->timed_dev.level = qpnp_hap_td_level;
	hap->timed_dev.get_level = qpnp_hap_td_get_level;
