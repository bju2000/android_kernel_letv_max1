...
struct panic_desc reason_arr[] =
{
	...
	{"ERR_IN_BOOT", TRIG_REASON_ENUM_MASK},//
};
...
int last_panic_reason_init(void)
{
	...
	if (((boot_reason == 1) || qpnp_pon_is_warm_reset()) && (tmp_rsn == TRIG_CMD_REBOOT)) //normal reboot
	...
	if ((tmp_rsn == TRIG_CMD_REBOOT) || (tmp_rsn == TRIG_CMD_POWEROFF))
		tmp_rsn = TRIG_REASON_ENUM_MASK;// a special value
...
late_initcall(panic_reason_init);
