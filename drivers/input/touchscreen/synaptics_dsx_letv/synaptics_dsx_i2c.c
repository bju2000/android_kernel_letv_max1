...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (prop && prop->length) {
		dev_err(dev, "we have found cap-button-codes node\n");
		bdata->cap_button_map->map = devm_kzalloc(dev,
				prop->length,
				GFP_KERNEL);
		if (!bdata->cap_button_map->map){
			dev_err(dev, "have no memory for button map\n");
			return -ENOMEM;
		}
		...
		dev_err(dev, "we don't have found cap-button-codes node\n");
		bdata->cap_button_map->nbuttons = 0;
		bdata->cap_button_map->map = NULL;
	}
...
#ifdef CONFIG_OF
	dev_err(&client->dev, "%s, device node\n",__func__);
...
#else
	dev_err(&client->dev, "%s, no device node\n",__func__);
